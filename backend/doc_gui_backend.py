...
# dependencies = ["python-docx", "openpyxl", "python-pptx", "pandas", "xlrd"]
# ///
"""
...
def _captured_run(cmd, label):
    ...
    try:
        ...
        if cmd and cmd[0] == "uvx":
            _CAP.append("✖ 未找到 uvx:docx → Markdown 依赖 uv(安装: brew install uv)")
        else:
            _CAP.append(f"✖ 命令不存在: {cmd[0] if cmd else '?'}")
        return 1
    if proc.stdout:
