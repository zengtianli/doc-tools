...
# ── lib path setup ──────────────────────────────────────────────────
sys.path.insert(0, str(Path(__file__).resolve().parent))  # 本地模块与本文件同目录
...
# 默认模板路径（与 md_docx_template.py 的 DEFAULT_TEMPLATE 一致）
DOCX_DEFAULT_TEMPLATE = str(Path(__file__).resolve().parent / "templates" / "template.docx")
