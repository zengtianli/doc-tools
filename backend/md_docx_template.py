...
sys.path.insert(0, str(Path(__file__).resolve().parent))  # 本地模块(file_ops 等)与本文件同目录
...
# 需要提取的样式 ID(默认中性模板的 styleId;自定义模板若含同 ID 样式亦会被提取)
TARGET_STYLES = {
    "Normal": "Normal",  # 基础样式
    "Heading1": "Heading 1",  # 标题1
    "Heading2": "Heading 2",  # 标题2
    "Heading3": "Heading 3",  # 标题3
    "Heading4": "Heading 4",  # 标题4
    "BodyText": "正文文本",  # 正文(可选,模板没有则退化到 Normal)
    "TableTitle": "表名",  # 表格标题(可选)
    "TableCell": "表格内容",  # 表格单元格文字(可选)
    "FigureTitle": "图名",  # 图片标题(可选)
}

# 样式依赖关系(basedOn 链,决定注入顺序)
STYLE_DEPS = {
    "Heading1": ["Normal"],
    "Heading2": ["Normal"],
    "Heading3": ["Normal"],
...
def extract_styles_xml(docx_path, output_dir=None):
    ...
    for style in root.findall(".//w:style", NSMAP):
        ...
        if style_id in TARGET_STYLES:
            ...
            if style_id == "BodyText":
                found_body_style = "BodyText"

        # 如果没有专用正文样式，用 Normal 作为正文
        if style_id == "Normal" and found_body_style is None:
            found_body_style = "Normal"
    ...
    with open(info_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"  表x ...   → {collected_styles.get('TableTitle', {}).get('name', 'Normal')}\n")
        f.write(f"  表格内容  → {collected_styles.get('TableCell', {}).get('name', 'Normal')}\n")
        f.write(f"  图x ...   → {collected_styles.get('FigureTitle', {}).get('name', 'Normal')}\n")
...
def convert_md_to_docx(md_path, styles_xml_path, output_path, config=None):
    ...
    # 表名/表格内容/图名:优先用模板提取到的真实样式名(模板里可能带空格),
    # 硬编码名只作 fallback —— 否则名字对不上就整体退化成正文样式。
    styles_map = (config or {}).get("styles", {})
    table_cell_style = styles_map.get("TableCell", "表格内容")
    table_title_style = styles_map.get("TableTitle", "表名")
    figure_title_style = styles_map.get("FigureTitle", "图名")
...
# 默认模板路径(随本后端一起分发的中性模板:Normal + Heading 1~4)
DEFAULT_TEMPLATE = str(Path(__file__).resolve().parent / "templates" / "template.docx")
